...
class Graph(object):
    ...
    def __add__(self, other: "Graph") -> "Graph":
        ...
        K = Graph(False, len(self.vertices) + len(other.vertices))
        d = {}
        for i in range(len(self.vertices)):
            d[self.vertices[i]] = K.vertices[i]
        for k in self.edges:
            f = Edge(d[k.tail], d[k.head])
            K.add_edge(f)
        for j in range(len(other.vertices)):
            d[other.vertices[j]] = K.vertices[j+len(self.vertices)]
