...
class MyGraph(object):
    ...
    def __add__(self, other: "Graph") -> "Graph":
        ...
        """
        newgraph = Graph(self.directed or other.directed)
        vertex_map = dict()
        for vertex in self.vertices:
            new_vertex = Vertex(newgraph, vertex.label)
            vertex_map[vertex] = new_vertex
            newgraph.add_vertex(new_vertex)
        for vertex in other.vertices:
            new_vertex = Vertex(newgraph, vertex.label)
            vertex_map[vertex] = new_vertex
            newgraph.add_vertex(new_vertex)
        for edge in self.edges:
            new_edge = Edge(vertex_map.get(edge.tail), vertex_map.get(edge.head))
    ...
    def del_edge(self, edge: "Edge"):
        self._e.remove(edge)
        set: "Set" = edge.tail._incidence.get(edge.head)
        set.remove(edge)
        set: "Set" = edge.head._incidence.get(edge.tail)
        set.remove(edge)
